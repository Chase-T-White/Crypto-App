  ...
  async (pageNumber: number, { getState }) => {
    ...
    if (selectedCurrency === "") {
      ...
      const response = await axios(
        `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${selectedCurrency}&order=market_cap_desc&per_page=20&page=${pageNumber}&sparkline=true&price_change_percentage=1h%2C24h%2C7d`
      );
...
export const { clearCoinsList } = coinsSlice.actions;
